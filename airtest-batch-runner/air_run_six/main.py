...
class CustomAirtestCase(AirtestCase):
    ...
    def get_root_log_path(self, report_name):  # 获取log总路径
        time_now_is = f"{datetime.now().strftime('%Y%m%d%H%M%S')}"
    ...
    def create_case_log(self, log):
        ...
        if os.path.isdir(log):
            ...
            output_file1 = os.path.join(log, 'log.html')
        return output_file1
    ...
    def reboot_adb(self):
        ...
        time.sleep(120)
        while f"{DEVICES_URL}\tdevice" != os.popen("adb devices").read().split("\n")[1]:
            print("设备adb连接异常，正在尝试重新连接")
            os.system(f"adb kill-server")
            time.sleep(5)
            os.system(f"adb start-server")
            time.sleep(5)
            os.system(f"adb connect {DEVICES_URL}")
            time.sleep(10)
    ...
    def run_air(self, device=devices):  # 聚合结果
        ...
        start_time_all = datetime.now().strftime('%Y-%m-%d %H:%M:%S')  # 获取当前用例运行的开始时间，写在报告的表头概况中
        ...
        for number in range(0, len(cases_total_list)):  # 运行获取到的每一条用例
            ...
            args = Namespace(device=device, log=log, recording=None, script=cases_total_list[number], compress=0,
                             no_image=False, scale=0.5)

            try:
                self.setUp()
                start_time = int(time.time())  # 获取每条用例运行的起始时间，用来获取用例运行时长
                time_now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')  # 获取用例的运行起始时间，写在报告中
                ...
                with io.open(output_file, 'w', encoding="utf-8") as f:
                    f.write(html)
                time.sleep(10)
                if (number + 1) % self.reboot_num == 0 and number != 0:
                    print(f"运行完第{number+1}条后正在重启")
                    self.reboot_adb()
