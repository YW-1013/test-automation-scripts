...
while True:
    try:
        ...
        check_and_reconnect_except(device_ip, power_ip, power_sn, power_path, power_process_name, power_name, logger)
        continue
    try:
        ...
        if match_image("template1.png", screenshot_path, threshold=0.8) and match_image("template2.png", screenshot_path, threshold=0.8):
            print(f"第{test_times}轮测试正常")
        else:
            print(f"第{test_times}轮测试不通过，测试停止")
            break

        switch_power(test_times, device_ip, power_ip, power_sn, power_path, power_process_name, power_name)
        test_times += 1
        time.sleep(120)
    except:
        continue