...
5、2024-12-25更新V1.5: （1）修改循环任务没有删除下发的任务的bug；（2）修改状态检测的配置项未保存的bug
"""
import os
import sys
...
def save_config():
    config = {
        ...
        "status_check": status_check_var.get()
    }
...
def load_config():
    if os.path.exists(CONFIG_FILE):
        with open(CONFIG_FILE, 'r') as f:
            ...
            status_check_var.set(config.get("status_check", "否"))
...
def delete_weekly_task(env_select, headers, task_num):
    ...
    status = requests.delete(url_delete, headers=headers).status_code
